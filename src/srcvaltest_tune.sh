...
perl -ne '/Y (.*) Z/; $this=$1; if ((!$last || $last ne $this)) { $n++; if ($n % 100 != 5) {print}}; $last=$this' $f | shuf | head -n 100000 > all_test.txt
...
perl -ne '/Y (.*) (.*) Z/ || die; $c[int($1/5)][int($2/5)] += 1; if ($. == 100000) { for ($i=0; $i< 25; $i++) { printf "%3d ",$i*5; for ($j=1; $j < 21; $j++) { if (!$i) { printf "%4d",$j*5; } else { printf "%4d",$c[$i][$j]+0 } } print "\n"; } }' all_test_possible.txt > table.txt
