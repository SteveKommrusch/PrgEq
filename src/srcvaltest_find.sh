...
cat reversed/tune10_2.txt reversed/tune10_20.txt | perl -ne '/^Legal axiom prop/ && ($l=1); if ($l) {if (/FOUND: (.*) with\s+(.*) Target path:\s+(.*)$/) { $t=$3; $f=$2; $p=$1; $n=scalar split /stm/,$f; s/FOUND:/X/; s/ to / Y /; s/ with\s+/ Z /; s/Target path:.*$//; if (! exists $fndsml{$p}) {print} else { $ax[$n]++; if (/stm20/ || /stm19/ || / N[lr][lr][lr]/ || / Newtmp/ || / Factor/) {print} else {$ax[$n] <= ($n*5) && print }}}} else {/FOUND: (.*) with/ && ($fndsml{$1}=1);}' >> raw_tune.txt
