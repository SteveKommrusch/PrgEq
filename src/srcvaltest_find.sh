...
for i in b1 b2 b3 b4 b5 b6 b7 b8 b9 b10; do
  cat $i/tune25_2.txt $i/tune25_20.txt | perl -ne '/^Legal axiom prop/ && ($l=1); if (/FOUND: (.*) with\s+(.*) Target path:\s+(.*)$/) { $t=$3; $f=$2; $p=$1; $n=scalar split /stm/,$f; if ($l) {s/FOUND:/X/; s/ to / Y /; s/ with\s+/ Z /; s/Target path:.*$//; if (! exists $fndsml{$p} && $n <= (2+ scalar split /stm/,$t)) {print} elsif ($n < (scalar split /stm/,$t)) { $ax[$n]++; if (/stm20/ || /stm19/ || / N[lr][lr][lr]/ || / Newtmp/ || / Factor/) {print} else {$ax[$n] <= ($n*5+10) && print }}} else {$n <= (2+ scalar split /stm/,$t) && ($fndsml{$p}=1);}}' >> raw_tune.txt
...
cat reversed/tune10_2.txt reversed/tune10_20.txt | perl -ne '/^Legal axiom prop/ && ($l=1); if ($l) {if (/FOUND: (.*) with\s+(.*) Target path:\s+(.*)$/) { $t=$3; $f=$2; $p=$1; $n=scalar split /stm/,$f; s/FOUND:/X/; s/ to / Y /; s/ with\s+/ Z /; s/Target path:.*$//; if (! exists $fndsml{$p}) {print} else { $ax[$n]++; if (/stm20/ || /stm19/ || / N[lr][lr][lr]/ || / Newtmp/ || / Factor/) {print} else {$ax[$n] <= ($n*10-20) && print }}}} else {/FOUND: (.*) with/ && ($fndsml{$1}=1);}' >> raw_tune.txt
