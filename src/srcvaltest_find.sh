...
if [[ ! -z $1 || ! -f "b10/tune25_2.txt" ]]; then
...
for i in b1 b2 b3 b4 b5 b6 b7 b8 b9 b10 reversed; do
  cat $i/tune??_2.txt $i/tune??_20.txt | perl -ne '/^Legal axiom prop/ && ($l=1); if (/FOUND: (.*) with\s+(.*) Target path:/) { $f=$2; $p=$1; $n=scalar split /stm\d/,$f; if ($l) {s/FOUND:/X/; s/ to / Y /; s/ with\s+/ Z /; s/Target path:.*$//; if (! exists $fndsml{$p}) {print} elsif ($n <= $fndsml{$p} - 2) { $ax[$n]++; if (/stm20/ || /stm19/ || / N[lr][lr][lr]/ || / Newtmp/ || / Factor/) {print} else {$ax[$n] <= ($n*7) && print }}} else {$fndsml{$p}=$n}}' >> raw_tune.txt
