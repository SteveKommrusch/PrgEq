...
perl -ne '/Y (.*) Z/; $this=$1; if ((!$last || $last ne $this)) { $n++; if ($n % 20 == 5) {print}}; $last=$this' $f | head -n 1000 > all_test.txt

perl -ne '/^(.*) X / && print $1."\n"' all_trainval.txt | head -n 500000 > src-train.txt
perl -ne '/^(.*) X / && print $1."\n"' all_trainval.txt | tail -n 1000 > src-val.txt
perl -ne '/^(.*) X / && print $1."\n"' all_test.txt > src-test.txt
perl -ne '/X (.*) Z / && print $1."\n"' all_trainval.txt | head -n 500000 > src-trainx.txt
perl -ne '/X (.*) Z / && print $1."\n"' all_test.txt > src-testx.txt
perl -ne '/X (.*) Z / && print $1."\n"' all_trainval.txt | tail -n 1000 > src-valx.txt
perl -ne '/Z (.*)\s*$/ && print $1."\n"' all_trainval.txt | head -n 500000 > tgt-train.txt
